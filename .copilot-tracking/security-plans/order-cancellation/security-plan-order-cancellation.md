...
| T-ORDERINGAPI-006 | Information Disclosure | Added 2026-09-16 (P02 validation finding 5). With the ownership check in place, distinct `404` (not found) and `403` (not owner) responses would let any authenticated customer probe the sequential `int` order numbers and enumerate which orders exist and how many are placed. Mitigated by the ADR amendment: the API returns `404` with an identical body for both cases; the handler, audit log, and metrics keep the distinction | REQ-002 | M | L | Low (mitigated) |
